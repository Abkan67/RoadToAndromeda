...
{name:"Nothing At All", description:"In the next leg of your journey is nothing at all, a long space of nothing.", func:()=>{game.nextEvent(()=>{if(wagonband.supplies.Engines>1){game.dialogue("With your multiple Engines you cruise past it");}else{wagonband.loseItem.Gas(); game.dialogue("With only one engine, you have to use some extra gas to get past it.");}})}, rarity:1},
...
{name:"Buyer", description:"You find a merchant, who asks to buy an item from you. He asks for A Tire for 200, An Engine for 630, A Radio for 430, or a character for 550 credits.", func:()=>{var choicesArray=[{name:"Don't Sell",func:'game.dialogue("You both go on your way")'}];var prices={Tires:200,Engines:630,Radio:430};if(wagonband.characters.length>=1){prices[randomArray(wagonband.characters).type]=550};for(var item in prices){if(wagonband.supplies[item]>=1){choicesArray.push({name:item, func:`wagonband.loseItem['${item}']();for(var i=0; i<${prices[item]};i++){wagonband.addItem.Credits();};`})};};game.createChoice(choicesArray);},rarity:2},
